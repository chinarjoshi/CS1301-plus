 ...
 *         - All of these functions must be completed in ONE line. That means
 *         they
 *         should be of the form "return [...];". No partial credit will be
 *         awarded
 ...
 *         - You may not use bit shifting or the exclusive OR operator (^) in
 *         xor.
 ...
 *         in a
 *         number (which is of type int by default) into a Method accepting a
 *         byte, then
 *         the Java compiler will complain even if the number would fit into
 *         that type.
 *
 *         Now, keep in mind the return value is also an int. Please read the
 *         comments
 *         about how many significant bits to return and make sure that the
 *         other bits
 *         are not set or else you will not get any points for that test case.
 *         For
 *         example, if we ask you to return 6 bits and you return "0xFFFFFFFF",
 *         you will
 *         lose points.
 *
...
public class Operations {
    ...
    int getShort(int num, int which) {
        return ((num & (0xFFFF << (which << 4))) >> (which << 4));
    }
